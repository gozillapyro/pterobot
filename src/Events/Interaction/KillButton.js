...
module.exports = async(interaction, Discord) => {
    ...
    if (hasKey === false) {
        ...
        .setAuthor({name:`Not Registered - Kill`,iconURL:interaction.guild.iconURL() ? interaction.guild.iconURL({ dynamic: true }) : ""})
        .setDescription(`Your account is not registered with a valid API key.\nTo register please run \`/register\`.`)
		.setFooter({text:`${embed.symbols.copyright} ${mainConfig.panel.company_name} - Kill ${embed.symbols.circle} ${user.tag}`,iconURL:user.displayAvatarURL({ dynamic: true })})
        .setTimestamp()
    ...
    let serverId = interaction.customId.replace('kill_', '');
    if (!serverId) return;

    const res1 = await pteroClient.setPowerState(serverId, "kill");

    if (res1 === 404||res1 === 403) {
        let permissionsEmbed = new Discord.MessageEmbed()
        .setAuthor({name:`Kill - Insuffient Permissions`, iconURL:interaction.guild.iconURL() ? interaction.guild.iconURL({ dynamic: true }) : ""})
        .setDescription(`You don't have the required permission to kill this server.`)
        .setFooter({ text: `${embed.symbols.copyright} ${mainConfig.panel.company_name} - Kill ${embed.symbols.circle} ${user.tag}`, iconURL: user.displayAvatarURL({ dynamic: true }) })
        .setTimestamp()
        .setColor(embed.colours.red)
    ...
    let killServerEmbed = new Discord.MessageEmbed()
    .setAuthor({name:`Kill Server`, iconURL:interaction.guild.iconURL() ? interaction.guild.iconURL({ dynamic: true }) : ""})
