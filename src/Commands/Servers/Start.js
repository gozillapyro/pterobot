...
module.exports = {
    ...
    async execute(client, Discord, mainConfig, interaction, args) {
        ...
        let res = await pteroClient.setPowerState(serverId, "start");
        if (res === 404||res === 403) {
        ...
        let startServerEmbed = new Discord.EmbedBuilder()
        .setAuthor({name:`Start Server`, iconURL:guild.iconURL() ? guild.iconURL({ dynamic: true }) : ""})
