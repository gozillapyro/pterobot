...
module.exports = {
    ...
    async execute(client, Discord, mainConfig, interaction, args) {
        ...
        let res = await pteroClient.setPowerState(serverId, "stop");
        if (res === 404||res === 403) {
        ...
        let stopServerEmbed = new Discord.EmbedBuilder()
        .setAuthor({name:`Stop Server`, iconURL:guild.iconURL() ? guild.iconURL({ dynamic: true }) : ""})
